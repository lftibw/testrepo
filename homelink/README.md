...
- **Web remote** (`/remote`): a control dashboard in the browser — power, brightness, warmth, color, and full AC controls (mode + temperature). Extras: master actions (**all lights on/off**, **everything off**), device-type **filters**, one-tap **color presets**, and per-device **auto-off timers** (15/30/60 min with a live countdown). Any change is sent to the device *and* mirrored into HomeKit, so the web remote and the Home app always agree. Because it talks straight to the device clouds, it works from anywhere you can reach the HomeLink server — no home hub required (unlike Apple's away-from-home access). Reachable from your phone at `http://<your-mac-name>.local:8580/remote`.
